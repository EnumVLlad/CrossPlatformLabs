...
public static class SecondLab
{
    ...
    public static void Execute(string inputFilePath, string outputFilePath)
    {
        ...
        try
        {
            (count, numbers) = ReadNumbersFromFile(inputFilePath);
        }
        catch (Exception e)
        {
        ...
        {
            Console.WriteLine($"Error while solving task: {e.Message}");
            return;
        }

        try
        {
            WriteResultToFile(solution, outputFilePath);
        }
        catch (Exception e)
        {
    ...
    private static (int Count, List<int> Numbers) ReadNumbersFromFile(string inputFilePath)
    {
        ...
        if (lines.Length != 2)
        {
            throw new InputException("File contains more than two lines with data");
        }
    ...
    private static void WriteResultToFile(int num, string outputFilePath)
    {
        File.WriteAllText(outputFilePath, num.ToString(CultureInfo.InvariantCulture));
    }
}
